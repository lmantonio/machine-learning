...
def set_costFunction(point_theta,matrix_features,vector_outputs):
    ...
    for i in range(0,numTrainingExamples):
        z = np.dot(matrix_features[i],point_theta)
        try:
            h = 1/(1+math.exp(-z))
        ...
        #sum = sum + (vector_outputs[i]*math.log10(h)+(1-vector_outputs[i])*math.log10(1.0-h))
        
        if (vector_outputs[i]>0):
            sum = sum + math.log10(h)
        else:
            sum = sum + math.log10(1.0-h)
        
    
    costFunction = -sum/numTrainingExamples
...
#------------------------------------------------------------------------------
X,Y = get_trainingSet('trainingset-log.txt')
...
log_modelCoef(modelcoef,meanX,stdX,"modelcoef-log.txt")
